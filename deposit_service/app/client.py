...
class MediaCMSInternalClient:
    ...
    def get_deposit_address_stats(self, options: list[dict]) -> list[dict]:

        result = self.post_signed("/api/internal/ledger/deposit-addresses/stats", {"options": options})

        return result["results"]


    def provision_deposit_addresses(self, address_rows: list[dict]) -> dict:
