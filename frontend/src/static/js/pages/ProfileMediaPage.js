...
export class ProfileMediaPage extends Page {
  ...
  authorDataLoad() {
    ...
    if (author) {
      if (this.state.query) {
        requestUrl = ApiUrlContext._currentValue.search.query + this.state.query + '&author=' + author.id + '&sort_by=add_date&ordering=desc';
      } else {
        requestUrl = ApiUrlContext._currentValue.media + '?author=' + author.id + '&sort_by=add_date&ordering=desc';
      }
    }
    if ('all' !== this.state.visibilityFilter){
        requestUrl += '&state=' + this.state.visibilityFilter;
    }
    this.setState({
      author: author,
  ...
  changeRequestQuery(newQuery) {
    ...
    if (newQuery) {
      requestUrl = ApiUrlContext._currentValue.search.query + newQuery + '&author=' + this.state.author.id + '&sort_by=add_date&ordering=desc';
    } else {
      requestUrl = ApiUrlContext._currentValue.media + '?author=' + this.state.author.id + '&sort_by=add_date&ordering=desc';
    }
  ...
  onVisibilityChange(e){
     ...
     this.setState({ visibilityFilter: v }, () => this.changeRequestQuery(this.state.query));
  }
  pageContent() { const authorData = ProfilePageStore.get('author-data');
