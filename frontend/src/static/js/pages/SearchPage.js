...
export class SearchPage extends Page {
  constructor(props) {
    ...
    this.state = {
      ...
      searchCelebrities: SearchFieldStore.get('search-celebrities'),
  ...
  updateRequestUrl() {
    const validQuery = this.state.searchQuery || this.state.searchCategories || this.state.searchTags || this.state.searchCelebrities;
    ...
    if (null !== this.state.resultsCount) {
      if (!validQuery) {
        ...
        } else if (this.state.searchCelebrities ) {
          title = null === this.state.resultsCount || 0 === this.state.resultsCount ? 'No' : this.state.resultsCount;
          title += ' ' + translateString('media in celebrity') + ' "' + this.state.searchCelebrities + '"';
        } else {
    ...
    const api_url_postfix =
      ...
      (this.state.searchCelebrities ? '&e=' + this.state.searchCelebrities : '') +
