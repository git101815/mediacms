...
function itemPageLink(props, item) {
  ...
  if (props.inCelebritiesList) {
    return LinksContext._currentValue.search.celebrity + item.title.replace(' ', '%20');
  }
  if (props.inTagsList) {
...
export function listItemProps(props, item, index) {
  const isArchiveItem = props.inCategoriesList || props.inTagsList || props.inCelebritiesList;
  ...
  if (isArchiveItem) {
    ...
    else if(props.inCelebritiesList){
      taxonomyPage.type = 'celebrities';
    }
...
export function ListItem(props) {
  ...
  if (props.taxonomyPage.current) {
    switch (props.taxonomyPage.type) {
      ...
      case 'celebrities':
        return <TaxonomyItem {...args} type="celebrities" />;
    }
  }
