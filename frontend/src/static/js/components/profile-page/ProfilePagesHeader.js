      ...
      <span className="profile-banner-wrap">
        {props.author.banner_thumbnail_url ? (
          <span
            ...
            style={{
              backgroundImage:`url(${props.author.banner_thumbnail_url})`,
            }}
          ></span>
        ) : null}
        ...
        {props.author.thumbnail_url || props.author.name ? (
          <div className="profile-info">
            <div className="profile-info-inner">
              ...
              <div>
                {props.author.name ? (() => {
                  const n = Number(props.author.media_count);
                  const show = Number.isFinite(n) && n >= 0;
                  return (
                    <h1>
                     {props.author.name}
                     {show && (<span className="profile-media-count-inline">{' '}· {n} {n === 1 ? 'video' : 'videos'}</span>)}
                    </h1>
                  );
                })() : null}
