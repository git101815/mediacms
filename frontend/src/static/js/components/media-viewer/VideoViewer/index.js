...
function loadVjsPluginNoAMD(url) {
  return new Promise((resolve, reject) => {
    ...
    s.async = false;
    s.onload = () => {
      if (hadAMD) window.define = oldDefine;
      resolve();
    };
    s.onerror = (e) => {
      if (hadAMD) window.define = oldDefine;
...
if (!window.__vjsPluginsLoadedPromise) {
  const PLUGINS_BASE = '/static/lib/video-js/plugins/';
  window.__vjsPluginsLoadedPromise = (async () => {
    await loadVjsPluginNoAMD(PLUGINS_BASE + 'videojs-contrib-ads.min.js');
    await loadVjsPluginNoAMD(PLUGINS_BASE + 'videojs.ima.min.js');
  })();
}
...
export default class VideoViewer extends React.PureComponent {
  ...
  render() {
    ...
    const previewSprite = !!this.props.data.sprites_url
      ? {
          url: /^(?:https?:)?\/\//i.test(this.props.data.sprites_url)
          ? this.props.data.sprites_url
          : this.props.siteUrl.replace(/\/$/, '') + '/' + this.props.data.sprites_url.replace(/^\//, ''),
