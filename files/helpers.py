...
def get_default_state(user=None):
    # possible states given the portal setting
    state = "private"
    if settings.DEFAULT_VISIBILITY == "public":
        state = "public"
    if settings.DEFAULT_VISIBILITY == "unlisted":
        state = "unlisted"
    if settings.DEFAULT_VISIBILITY == "private_verified":
...
def url_from_path(filename):
    ...
    if isinstance(filename, str):
        ...
        if f.startswith("http://") or f.startswith("https://") or f.startswith("//"):
            return f
        rel = f.replace(settings.MEDIA_ROOT, "").lstrip("/")
        return urljoin(settings.MEDIA_URL, rel)
    return None
...
def media_file_info(input_file):
    ...
    for video in video_streams:
        rfr = video.get("r_frame_rate", "0/0").split("/",1)
        try:
            n, d = int(rfr[0]), int(rfr[1])
        ...
        if d > 0 and n > 0:
            ...
            video_info = video
    if info.get("format") and info["format"].get("format_name", "") in ["tty","image2","image2pipe","bin","png_pipe","gif"]:
        ret["fail"] = True
        return ret
...
def get_base_ffmpeg_command(
    ...
    if pass_number == 1:
        ...
        if output_file.endswith("mp4") and codec in ("h265", "hevc"):
...
def get_alphanumeric_only(string):
    ...
    string_clean = "".join([char for char in string if char in allowed_chars])
    return string_clean
