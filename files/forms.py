...
from .models import MEDIA_STATES, Category, Media, Subtitle, Celebrity
...
class MediaMetadataForm(forms.ModelForm):
    ...
    def clean_uploaded_poster(self):
        if "uploaded_poster" not in self.changed_data:
            return self.cleaned_data.get("uploaded_poster")
    ...
    def __init__(self, user, *args, **kwargs):
        ...
        if getattr(settings, 'USE_RBAC', False) and 'category' in self.fields:
            if is_mediacms_editor(user):
                ...
                self.fields['celebrities'].queryset = Celebrity.objects.order_by(Lower('title'))

        if 'category' in self.fields:
            qs = self.fields['category'].queryset
            self.fields['category'].queryset = qs.select_related("group").order_by("group__ordering", Lower("group__title"), Lower("title"))
        if 'celebrities' in self.fields:
            qs = self.fields['celebrities'].queryset
            self.fields['celebrities'].queryset = qs.select_related("group").order_by("group__ordering", Lower("group__title"), Lower("title"))
        if self.instance and self.instance.pk:
            if 'category' in self.fields:
                self.fields['category'].initial = [str(pk) for pk in self.instance.category.values_list('id', flat=True)]
            if 'celebrities' in self.fields:
                self.fields['celebrities'].initial = [str(pk) for pk in self.instance.celebrities.values_list('id', flat=True)]
        ...
        self.helper.form_show_errors = False
        self.helper.layout = Layout(
            CrispyField('category', template='cms/crispy_categories_grouped.html'),
            CrispyField('celebrities', template='cms/crispy_celebrities_grouped.html'),
