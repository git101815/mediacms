# ledger/management/commands/recover_orphan_deposit_addresses.py
...
SUPPORTED_STABLECOIN_ASSETS = {"USDT", "USDC"}
...
def _load_runtime_config_from_path(
    *,
    ...
    for item in raw_options:
        funding_private_key = _normalize_private_key(_read_secret_file(item["funding_private_key_file"]))
        destination_address = str(_resolve_env_placeholder(item.get("destination_address", ""))).strip().lower()
...
def _stablecoin_value_usd(*, asset_code: str, raw_amount: int, chain: str) -> Decimal | None:
    if (asset_code or "").strip().upper() not in SUPPORTED_STABLECOIN_ASSETS:
        return None
...
def _get_candidate_queryset(*, chain: str, asset_code: str, addresses: list[str], session_public_ids: list[str], older_than_hours: int, force_recheck: bool):
    ...
    if addresses:
        qs = qs.filter(deposit_address__in=[_normalize_evm_address(v) for v in addresses])
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("--asset-code", default="")
        parser.add_argument("--address", nargs="*", default=[])
        parser.add_argument("--session-public-id", nargs="*", default=[])
        parser.add_argument("--older-than-hours", type=int, default=72)
        parser.add_argument("--max-addresses", type=int, default=100)
        parser.add_argument("--native-price-usd", required=True)
        parser.add_argument("--min-token-value-usd", default="5")
        parser.add_argument("--min-native-value-usd", default="2")
        parser.add_argument("--profit-multiplier", default="2")
    ...
    def _get_lock_actor(self):
        username = str(getattr(settings, "LEDGER_INTERNAL_SWEEPER_SERVICE_USERNAME", "") or "").strip()
        if not username:
            raise CommandError("LEDGER_INTERNAL_SWEEPER_SERVICE_USERNAME must be configured")
        user_model = get_user_model()
        actor = user_model.objects.filter(username=username).first()
        if actor is None:
            raise CommandError("Configured sweeper service actor does not exist")
        return actor

    def _sender_lock_seconds(self) -> int:
        value = int(getattr(settings, "LEDGER_EVM_SENDER_LOCK_SECONDS", 120) or 120)
        return max(30, value)

    def _pending_nonce(self, *, w3, address: str) -> int:
        return int(w3.eth.get_transaction_count(w3.to_checksum_address(address), block_identifier="pending"))
    ...
    def _broadcast_with_sender_lock(
        ...
        *,
        ...
        lock = acquire_evm_sender_lock(
            actor=actor,
            service_name=service_name,
            chain=chain,
            address=address,
            lock_seconds=self._sender_lock_seconds(),
        )
        lock_token = lock["lock_token"]
        backend_next_nonce = lock.get("next_nonce")
        rpc_pending_nonce = self._pending_nonce(w3=w3, address=address)
        selected_nonce = int(rpc_pending_nonce) if backend_next_nonce is None else max(int(backend_next_nonce), int(rpc_pending_nonce))

        txid = ""
        try:
            tx = tx_builder(selected_nonce)
            signed = sign_transaction(tx=tx, private_key=signer_private_key)
            txid = send_signed_transaction(
                w3=w3,
    ...
    def handle(self, *args, **options):
        ...
        addresses = [str(v).strip().lower() for v in options["address"] if str(v).strip()]
        session_public_ids = [str(v).strip() for v in options["session_public_id"] if str(v).strip()]
        ...
        if bool(reference_heads_base_url) != bool(reference_heads_shared_secret):
            raise CommandError("Provide both --reference-heads-base-url and --reference-heads-shared-secret, or neither.")
        ...
        self._lock_actor = self._get_lock_actor()
        self._lock_service_name = "orphan-recovery-command"
        ...
        candidates = list(_get_candidate_queryset(
            chain=chain,
    ...
    def _process_candidate(
        ...
        *,
        ...
        funding_txid = token_sweep_txid = native_sweep_txid = ""
        token_balance = native_balance = 0
        token_value_usd = native_value_usd = token_recovery_cost_usd = native_recovery_cost_usd = None

        try:
            option = runtime.option_index.get(_option_key(
                chain=session.chain,
            ...
            if option is None:
                ...
                _save_audit(
                    audit=audit, status=status, decision_reason="missing_route_option",
                    token_balance=0, native_balance=0, token_value_usd=None, native_value_usd=None,
                    token_recovery_cost_usd=None, native_recovery_cost_usd=None,
                    funding_txid="", token_sweep_txid="", native_sweep_txid="", error_message="",
                    metadata={"commit": commit, "planned_actions": [], "session_public_id": str(session.public_id)},
                )
                return status
            ...
            if derivation_index is None:
                status = OrphanDepositRecoveryAudit.STATUS_IGNORED_FINAL
                _save_audit(
                    audit=audit, status=status, decision_reason="missing_derivation_index",
                    token_balance=0, native_balance=0, token_value_usd=None, native_value_usd=None,
                    token_recovery_cost_usd=None, native_recovery_cost_usd=None,
                    funding_txid="", token_sweep_txid="", native_sweep_txid="", error_message="",
                    metadata={"commit": commit, "planned_actions": [], "session_public_id": str(session.public_id)},
            ...
            derived_address = runtime.deriver.derive_address(chain=option.chain, address_index=int(derivation_index)).lower()
            if derived_address != source_address:
                status = OrphanDepositRecoveryAudit.STATUS_IGNORED_FINAL
                _save_audit(
                    audit=audit, status=status, decision_reason="derivation_mismatch",
                    token_balance=0, native_balance=0, token_value_usd=None, native_value_usd=None,
                    token_recovery_cost_usd=None, native_recovery_cost_usd=None,
                    funding_txid="", token_sweep_txid="", native_sweep_txid="", error_message="",
                    metadata={"commit": commit, "planned_actions": [], "session_public_id": str(session.public_id)},
            ...
            source_private_key = runtime.deriver.derive_private_key(chain=option.chain, address_index=int(derivation_index))
            if option.destination_address == source_address:
                status = OrphanDepositRecoveryAudit.STATUS_IGNORED_FINAL
                _save_audit(
                    audit=audit, status=status, decision_reason="destination_equals_source",
                    token_balance=0, native_balance=0, token_value_usd=None, native_value_usd=None,
                    token_recovery_cost_usd=None, native_recovery_cost_usd=None,
                    funding_txid="", token_sweep_txid="", native_sweep_txid="", error_message="",
                    metadata={"commit": commit, "planned_actions": [], "session_public_id": str(session.public_id)},
            ...
            token_balance = int(get_erc20_balance(
                w3=w3, token_contract_address=option.token_contract_address, owner_address=source_address
            ))
            ...
            if token_balance == 0 and native_balance == 0:
                ...
                _save_audit(
                    audit=audit, status=status, decision_reason="empty_wallet",
                    token_balance=0, native_balance=0, token_value_usd=None, native_value_usd=Decimal("0"),
                    token_recovery_cost_usd=None, native_recovery_cost_usd=Decimal("0"),
                    funding_txid="", token_sweep_txid="", native_sweep_txid="", error_message="",
                    metadata={"commit": commit, "planned_actions": [], "selected_rpc_url": selected_rpc_url},
                )
                return status
            ...
            if token_balance > 0:
                token_value_usd = _stablecoin_value_usd(asset_code=session.asset_code, raw_amount=token_balance, chain=session.chain)
                if token_value_usd is None:
                    status = OrphanDepositRecoveryAudit.STATUS_IGNORED_FINAL
                    _save_audit(
                        audit=audit, status=status, decision_reason="unsupported_token_pricing",
                        token_balance=token_balance, native_balance=native_balance,
                        token_value_usd=None, native_value_usd=_native_wei_to_usd(amount_wei=native_balance, native_price_usd=native_price_usd),
                        token_recovery_cost_usd=None, native_recovery_cost_usd=None,
                        funding_txid="", token_sweep_txid="", native_sweep_txid="", error_message="",
                        metadata={"commit": commit, "planned_actions": [], "selected_rpc_url": selected_rpc_url},
                    )
                ...
                token_transfer_gas_limit = int(_estimate_erc20_transfer_gas(
                    w3=w3, option=option, source_address=source_address, amount=token_balance
                ))
            ...
            if token_profitable:
                estimated_native_after_token = 0 if token_topup_needed_wei > 0 else max(0, native_balance - token_transfer_fee_wei)

            native_recoverable_wei = max(0, estimated_native_after_token - native_transfer_fee_wei)
            native_value_usd = _native_wei_to_usd(amount_wei=native_recoverable_wei, native_price_usd=native_price_usd)
            native_recovery_cost_usd = _native_wei_to_usd(amount_wei=native_transfer_fee_wei, native_price_usd=native_price_usd)
            ...
            if not planned_actions:
                ...
                _save_audit(
                    audit=audit, status=status, decision_reason="below_profit_threshold",
                    token_balance=token_balance, native_balance=native_balance,
                    token_value_usd=token_value_usd, native_value_usd=native_value_usd,
                    token_recovery_cost_usd=token_recovery_cost_usd, native_recovery_cost_usd=native_recovery_cost_usd,
                    funding_txid="", token_sweep_txid="", native_sweep_txid="", error_message="", metadata=metadata,
                )
                return status
            ...
            if not commit:
                ...
                _save_audit(
                    audit=audit, status=status, decision_reason="|".join(planned_actions),
                    token_balance=token_balance, native_balance=native_balance,
                    token_value_usd=token_value_usd, native_value_usd=native_value_usd,
                    token_recovery_cost_usd=token_recovery_cost_usd, native_recovery_cost_usd=native_recovery_cost_usd,
                    funding_txid="", token_sweep_txid="", native_sweep_txid="", error_message="", metadata=metadata,
            ...
            if token_profitable:
                if token_topup_needed_wei > 0:
                    ...
                    if funding_wallet_balance < required_funding_wallet_budget_wei:
                        raise RuntimeError("Funding wallet does not have enough native balance for orphan recovery")

                    funding_txid = self._broadcast_with_sender_lock(
                        actor=self._lock_actor,
                        service_name=self._lock_service_name,
                        chain=option.chain,
                    ...
                    wait_for_confirmations(
                        w3=w3, txid=funding_txid, required_confirmations=option.funding_confirmations, timeout_seconds=option.tx_timeout_seconds
                    )

                token_sweep_txid = self._broadcast_with_sender_lock(
                    actor=self._lock_actor,
                    service_name=self._lock_service_name,
                    chain=option.chain,
                ...
                wait_for_confirmations(
                    w3=w3, txid=token_sweep_txid, required_confirmations=option.sweep_confirmations, timeout_seconds=option.tx_timeout_seconds
                )
                native_balance = int(get_native_balance(w3=w3, address=source_address))

            native_recoverable_after_exec_wei = max(0, native_balance - native_transfer_fee_wei)
            native_value_after_exec_usd = _native_wei_to_usd(amount_wei=native_recoverable_after_exec_wei, native_price_usd=native_price_usd)

            if native_recoverable_after_exec_wei > 0 and native_value_after_exec_usd >= min_native_value_usd:
                native_sweep_txid = self._broadcast_with_sender_lock(
                    actor=self._lock_actor,
                    service_name=self._lock_service_name,
                    chain=option.chain,
                    address=source_address,
                ...
                wait_for_confirmations(
                    w3=w3, txid=native_sweep_txid, required_confirmations=option.sweep_confirmations, timeout_seconds=option.tx_timeout_seconds
                )
                native_balance = int(get_native_balance(w3=w3, address=source_address))
            ...
            _save_audit(
                audit=audit, status=status, decision_reason="retryable_error",
                token_balance=token_balance, native_balance=native_balance,
                token_value_usd=token_value_usd, native_value_usd=native_value_usd,
                token_recovery_cost_usd=token_recovery_cost_usd, native_recovery_cost_usd=native_recovery_cost_usd,
                funding_txid=funding_txid, token_sweep_txid=token_sweep_txid, native_sweep_txid=native_sweep_txid,
                error_message=str(exc), metadata={"commit": commit, "session_public_id": str(session.public_id)},
            )
            return status