...
from ledger.providers.malum import (
    MALUM_CHAIN,
    MALUM_MIN_CANONICAL_STABLE_AMOUNT,
    MALUM_NETWORK_DISPLAY,
    MALUM_PAYMENT_METHOD_KEY,
    MALUM_PAYMENT_METHOD_LABEL,
    MALUM_PAYMENT_METHOD_TYPE,
    MALUM_PROVIDER_KEY,
    MALUM_STATUS_CANCELED,
    MALUM_STATUS_CANCELLED,
    MALUM_STATUS_COMPLETED,
    MALUM_STATUS_CREATED,
    MALUM_STATUS_EXPIRED,
    MALUM_STATUS_PROCESSING,
    build_malum_metadata_value,
    create_malum_payment,
    get_malum_currency,
    get_malum_payment_ttl_seconds,
    get_malum_public_base_url,
    lookup_malum_payment,
    malum_enabled,
    malum_private_key_is_sandbox,
    malum_route_key,
    parse_malum_metadata_value,
    verify_malum_webhook_signature,
)
...
def _build_absolute_url(path: str) -> str:
    base_url = get_malum_public_base_url()
    if not base_url:
...
def get_malum_deposit_option() -> dict | None:
    ...
    currency = get_malum_currency()
    return {
        "key": malum_route_key(currency),
        ...
        "session_ttl_seconds": get_malum_payment_ttl_seconds(),
...
def open_malum_deposit_session(
    *,
    ...
    currency = get_malum_currency()
    route_key = malum_route_key(currency)
    synthetic_ref = f"malum:{public_id.hex}"
    now = timezone.now()
    expires_at = now + timedelta(seconds=get_malum_payment_ttl_seconds())
    ...
    malum_metadata = build_malum_metadata_value(
    ...
    try:
        payment_response = create_malum_payment(
            amount=_canonical_stable_to_malum_amount(expected_canonical_amount),
            currency=currency,
...
def _get_malum_session_from_payload(payload: dict) -> DepositSession:
    ...
    metadata = parse_malum_metadata_value(payload.get("metadata"))
    ...
    if token_pack_snapshot:
        ...
        txn = apply_ledger_transaction(
            ...
            metadata={
                ...
                "requested_currency": get_malum_currency(),
...
def process_malum_webhook(payload: dict) -> dict:
    ...
    if status == MALUM_STATUS_COMPLETED:
        ...
        if requested_currency and requested_currency != get_malum_currency():
...
__all__ = [
    ...
    "credit_malum_deposit_session",
    "process_malum_webhook",
]
