...
@pytest.mark.django_db
def test_creator_can_disable_new_subscription_email(
    django_user_model,
    settings,
    django_capture_on_commit_callbacks,
):
    ...
    creator = create_user(
        django_user_model,
        "sub_creator_email_off",
        creator=True,
        email="sub-email-off@example.com",
    )
    creator.notification_on_new_subscriptions = False
    creator.save(update_fields=["notification_on_new_subscriptions"])
...
@pytest.mark.django_db
def test_renewal_email_is_controlled_by_creator_preference(
    django_user_model,
    settings,
    django_capture_on_commit_callbacks,
):
    ...
    creator.notification_on_subscription_renewals = True
...
@pytest.mark.django_db
def test_promotional_subscription_payment_stays_promotional_for_creator(
    django_user_model,
    settings,
):
    settings.PREMIUM_SUBSCRIPTION_CREATOR_SHARE_BPS = 8000
    ...
    creator = create_user(
        django_user_model,
        "sub_creator_promo_payment",
        creator=True,
    )
    subscriber = create_user(
        django_user_model,
        "sub_buyer_promo_payment",
    )
    plan = create_plan(creator, price_tokens=10 * TOKEN_SCALE)
    ...
    assert creator_wallet.balance == 8 * TOKEN_SCALE
    assert creator_wallet.promotional_balance == 4_800_000
    ...
    assert txn.metadata["creator_promotional_units"] == 4_800_000
    assert txn.metadata["creator_paid_units"] == 3_200_000
    assert txn.metadata["platform_promotional_consumed_units"] == 1_200_000
    assert txn.metadata["platform_paid_units"] == 800_000
