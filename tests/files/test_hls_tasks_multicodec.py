...
@pytest.mark.parametrize(
    ("codec", "task_name", "db_field", "fragment_prefix"),
    (
        ("h265", "create_hls_hevc_fmp4", "hls_hevc_file", "hevc"),
        ("av1", "create_hls_av1_fmp4", "hls_av1_file", "av1"),
    ),
)
def test_multicodec_hls_tasks_fragment_same_basename_inputs_to_unique_temp_files(
    ...
    assert len(fragmented_outputs) == 3
    assert len(set(fragmented_outputs)) == 3
...
@pytest.mark.django_db
@pytest.mark.parametrize(
    ("codec", "task_name", "db_field"),
    (
        ("h265", "create_hls_hevc_fmp4", "hls_hevc_file"),
        ("av1", "create_hls_av1_fmp4", "hls_av1_file"),
    ),
)
def test_multicodec_hls_tasks_wait_for_all_active_renditions_before_writing_master(
    ...
    profiles = {resolution: profile_factory(codec=codec, resolution=resolution) for resolution in (480, 720, 1080)}

    for resolution in (480, 720):
        rel_path = f"encoded/{codec}/{resolution}/same-output-name.mp4"
        full_path = media_root / rel_path
        full_path.parent.mkdir(parents=True, exist_ok=True)
        full_path.write_bytes(f"{codec}-{resolution}".encode("utf-8"))
...
@pytest.mark.django_db
@pytest.mark.parametrize(
    ("codec", "task_name", "db_field", "output_dir"),
    (
        ("h265", "create_hls_hevc_fmp4", "hls_hevc_file", "hevc"),
        ("av1", "create_hls_av1_fmp4", "hls_av1_file", "av1"),
    ),
...
def test_multicodec_hls_tasks_are_idempotent_when_master_already_exists(
    ...
    final_master = Path(hls_root) / media.uid.hex / output_dir / "master.m3u8"
