...
class TestDepositSessionViewsFunctional(BaseLedgerTestCase):
    def setUp(self):
        super().setUp()
    ...
    def _default_session_metadata(self):
        return {
            ...
            "expected_canonical_stable_amount": int(
                self.default_token_pack.gross_stable_amount
            ),
            "expected_route_raw_amount": int(
                self.default_token_pack.gross_stable_amount
            ),
        }

    def _create_bsc_route(self):
    ...
    def _create_bsc_pack(self, code, sort_order):
        return TokenPack.objects.create(
            code=code,
            name="500_tokens",
    ...
    def test_open_deposit_session_reuses_existing_active_session(
        self,
        mocked_derive,
    ):
    ...
    @patch("ledger.services._derive_session_deposit_address")
    def test_open_deposit_session_does_not_reuse_expired_session(
        self,
        mocked_derive,
    ):
    ...
    def test_deposit_session_page_context_contains_payment_payload(self):
        session = DepositSession.objects.create(
            user=self.u1,
            wallet=self.w1,
            chain="ethereum",
        ...
        payload = response.context["deposit_session"]
        ...
        self.assertEqual(
            payload["token_pack_name"],
            self.default_token_pack.name,
        )
        self.assertEqual(
            payload["expected_payment_currency"],
            "USDT",
        )
    ...
    @patch("ledger.services._derive_session_deposit_address")
    def test_credited_session_maps_to_payment_detected(
        self,
        mocked_derive,
    ):
        self.grant_perm(
        ...
        observed = record_onchain_observation(
            ...
            raw_payload={"source": "view-integration-test"},
        )
        credit_confirmed_deposit_session(
            actor=self.operator,
            deposit_session=session,
    ...
    @patch("ledger.services._derive_session_deposit_address")
    def test_bsc_pack_uses_canonical_and_raw_amounts(
        self,
        mocked_derive,
    ):
        mocked_derive.return_value = (
        ...
        self._create_bsc_route()
        bsc_pack = self._create_bsc_pack(
            "500_tokens",
            10,
        )
        self.client.force_login(self.u1)
        ...
        expected_raw = expected_canonical * 10**12

        self.assertEqual(session.chain, "bsc")
        self.assertEqual(session.min_amount, expected_canonical)
        self.assertEqual(
        ...
        self.assertEqual(
            int(
                (session.metadata or {})[
                    "expected_canonical_stable_amount"
                ]
            ),
            expected_canonical,
        )
        ...
        payload = page_response.context["deposit_session"]
        self.assertEqual(payload["token_pack_name"], bsc_pack.name)
        self.assertEqual(payload["min_amount"], expected_canonical)
        self.assertEqual(
            payload["expected_payment_currency"],
            "USDT",
        )
    ...
    @patch("ledger.services._derive_session_deposit_address")
    def test_bsc_underpayment_does_not_credit(
        self,
        mocked_derive,
    ):
        mocked_derive.return_value = (
        ...
        with self.assertRaisesMessage(
            Exception,
            "Observed canonical stable amount must be positive",
        ):
            record_onchain_observation(
                actor=self.operator,
                deposit_session=session,
    ...
    @patch("ledger.services._derive_session_deposit_address")
    def test_bsc_partial_payment_payload_uses_canonical_observed_amount(
        self,
        mocked_derive,
    ):
        mocked_derive.return_value = (
        ...
        self.assertEqual(payload["observed_amount"], 4_000_000)
        self.assertEqual(payload["confirmations"], 201)
        self.assertEqual(payload["status"], "payment_detected")
        self.assertEqual(
            payload["expected_payment_currency"],
            "USDT",
        ...
        self.assertEqual(
            status_response.json()["observed_amount"],
            4_000_000,
        )

    @patch("ledger.services._derive_session_deposit_address")
    def test_bsc_partial_payment_is_not_credited(
        self,
        mocked_derive,
    ):
        mocked_derive.return_value = (
